...
RU: dict[str, str] = {
    ...
    "File: one line VALUE1:VALUE2 or VALUE1|VALUE2. Passwords stay in RAM.":
        "Файл: одна строка VALUE1:VALUE2 или VALUE1|VALUE2. Пароли остаются в ОЗУ.",
    "TEST VARIABLE RESOLUTION": "ПРОВЕРИТЬ ПОДСТАНОВКУ ПЕРЕМЕННЫХ",
    "TXT PARSED": "TXT РАЗОБРАН",
    "CURRENT RECORD CREATED": "ТЕКУЩАЯ ЗАПИСЬ СОЗДАНА",
    "RUNTIME CONTEXT CREATED": "RUNTIME CONTEXT СОЗДАН",
    "PLACEHOLDER FOUND": "ПЛЕЙСХОЛДЕР НАЙДЕН",
    "PLACEHOLDER RESOLVED": "ПЛЕЙСХОЛДЕР ПОДСТАВЛЕН",
    "TYPE ACTION": "ДЕЙСТВИЕ TYPE",
    "Record loaded: #%s": "Запись загружена: №%s",
    "EMAIL exists: %s": "EMAIL есть: %s",
    "EMAIL length: %s": "EMAIL длина: %s",
    "PASSWORD exists: %s": "PASSWORD есть: %s",
    "PASSWORD length: %s": "PASSWORD длина: %s",
    "Template: %s": "Шаблон: %s",
    "Variable resolved: %s": "Переменная подставлена: %s",
    "Resolved length: %s": "Длина после подстановки: %s",
    "TYPE action started": "TYPE действие начато",
    "Refusing to type an unresolved placeholder":
        "Отказ вводить неподставленный плейсхолдер",
    "VERIFY": "ПРОВЕРКА",

...
async function main() {
  ...
  const store = new ChannelStateStore(baseDir);
  const { discovered } = store.syncFromProfileMapping(mapping);

  console.log(`\n🔄 Оркестратор: ${profileIds.length} каналов в конфиге`);
  if (discovered.new.length) {
    console.log(`🆕 Новые каналы (${discovered.new.length}): ${discovered.new.map((c) => `№${c.channelNumber}`).join(', ')}`);
  }
  if (discovered.existing.length) {
    console.log(`✅ Известные каналы (${discovered.existing.length}): ${discovered.existing.map((c) => `№${c.channelNumber}`).join(', ')}`);
  }
